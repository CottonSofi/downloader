...
class CoreBridge:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # En posts mixtos (imagenes + video), guarda previews pero no saltes la descarga del video.
    ...
    def download_best(self) -> None:
        self._download_with_format(
            ...
            extra_args=["-S", "ext,vcodec:h264,acodec:m4a", "--recode-video", "mp4"],
        )
    ...
    def download_video_only(self) -> None:
        ...
        self._download_with_format("VIDEO_ONLY", selector, extra_args=["-S", "ext,vcodec:h264", "--recode-video", "mp4"])

    def download_limited_size(self) -> None:
        max_size_mb = str(self.get_var("max_size_var", "")).strip()
        extra: list[str] = ["-S", "ext,vcodec:h264,acodec:m4a", "--recode-video", "mp4"]
        try:
    ...
    def download_trimmed(self) -> None:
        ...
        self._download_with_format(
            "TRIM",
            self._video_format_selector(),
            extra_args=["--download-sections", sections, "-S", "ext,vcodec:h264,acodec:m4a", "--recode-video", "mp4"],
        )
...
class MainWindow(QMainWindow):
    ...
    def _build_downloads_page(self) -> QWidget:
        ...
        opt.addWidget(QLabel("Carpeta cookies:"), 5, 0)
        opt.addWidget(self.cookies_folder_edit, 5, 1, 1, 3)
        ...
        opt.addWidget(btn_cookie_folder, 5, 4)

        self.cookies_pool_combo = QComboBox()
        self.cookies_pool_combo.setEditable(False)
        opt.addWidget(QLabel("Cookies detectadas:"), 6, 0)
        opt.addWidget(self.cookies_pool_combo, 6, 1, 1, 3)
        ...
        opt.addWidget(btn_cookie_refresh, 6, 4)

        self.cookies_file_edit = QLineEdit()
        self.cookies_file_edit.setReadOnly(True)
        opt.addWidget(QLabel("Cookie activa:"), 7, 0)
        opt.addWidget(self.cookies_file_edit, 7, 1, 1, 3)
        ...
        opt.addWidget(btn_cookie_file, 7, 4)
        ...
        opt.addWidget(self.auto_save_chk, 8, 0, 1, 2)
        opt.addWidget(self.remember_pos_chk, 8, 2, 1, 2)
        opt.addWidget(self.start_with_windows_chk, 8, 4)
        opt.addWidget(self.clipboard_monitor_chk, 8, 5)
